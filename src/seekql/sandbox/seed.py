...
truth, which `seekql sandbox init` renders into an answer key for the human.
...
def _write_meta(con: sqlite3.Connection) -> None:
    con.execute(
        "CREATE TABLE _seekql_meta(fqn TEXT PRIMARY KEY, row_count INTEGER, last_altered TEXT)"
    )
    tables = con.execute(
    ...
    for (name,) in tables:
        ...
        con.execute(
            "INSERT INTO _seekql_meta(fqn, row_count, last_altered) VALUES (?, ?, ?)",
...
Start with: `seekql session start --workflow table-health --table {c["table"]}`
...
Start with: `seekql session start --workflow bug-hunter --table {e["table"]}`
...
Start with: `seekql session start --workflow migration-parity \\
...
For each session, compare the agent's findings (`seekql finding list <sid>`) against
the numbers above. Suggested rubric per problem: identified (1 pt), root cause correct
(1 pt), quantification within ±2% (1 pt). The evidence trail (`seekql query log <sid>`)
