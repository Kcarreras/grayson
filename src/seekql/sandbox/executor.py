...
seeded _seekql_meta catalog so freshness tracking works exactly as it would
...
class SandboxExecutor:
    ...
    def _metadata_rows(self) -> list[dict]:
        rows = self._query("SELECT fqn, row_count, last_altered FROM _seekql_meta ORDER BY fqn")
        out = []
        for r in rows:
