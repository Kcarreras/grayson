...
Teams already run scheduled deterministic checks outside seekql — Airflow DAGs,
...
library like knowledge and views), and seekql surfaces the latest result per
...
seekql never runs these checks; it only reads, validates, and reports them.
"""
...
from seekql.util import read_json, write_json
...
data-quality jobs, anything scheduled. seekql reads every `*.json` under this
...
or pipe results through `seekql checks ingest <file>` which validates and keeps
