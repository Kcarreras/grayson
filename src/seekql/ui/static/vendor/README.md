...
a CDN link. Served from loopback by `seekql.ui.server.static_asset`.
...
Browsers cache these for a year, keyed on the `?v=<seekql version>` the
templates append, so an upgrade must also bump `seekql.__version__` to be picked
