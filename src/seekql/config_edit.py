"""Deliberate, validated edits to seekql.toml — the user-facing settings surface.

seekql.toml holds the rails (guard profiles, scopes, connection, library
pointer), so edits go through here with validation, and only via human
surfaces: the `seekql config` CLI and the console's Settings page. The MCP
...
from seekql.config import BUILTIN_PROFILES, CONFIG_FILENAME, GuardSettings, WorkspaceConfig
...
def set_values(root: Path, changes: dict[str, Any]) -> dict:
    """Apply {dotted_key: value} changes to seekql.toml. Returns what changed."""
