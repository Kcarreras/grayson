...
Runs statements through the `snow` CLI using a named connection. grayson never
...
SNOW_CMD_ENV = "GRAYSON_SNOW_CMD"  # JSON list overriding the snow binary (tests, wrappers)
...
def get_executor(connection: str, workspace_root: Path | None = None) -> Executor:
    if connection == "sandbox" and workspace_root is not None:
        # Local mock warehouse (no snow CLI): see grayson.sandbox.
        from grayson.sandbox.executor import SandboxExecutor, locate_warehouse
