...
Axis labels never collide and never hide what varies: how many category labels
are drawn, and how long, is computed from the plot width (skipped, never
overlapped); the affix every label shares is stripped and captioned once, dotted
identifiers shorten from the front, and any label still cut carries its full text
in the markup (`<title>` + `data-full`) for the console's hover tip and for
exported files. Bar charts with many categories or long names render as
horizontal bars (one row per category; dates and numbers stay vertical; the
spec's `orientation` forces either), the tile showing the rows that fit with a
`+N more` footer and the detail size growing to hold them all. Two layouts render
from the same data — the tile (session page, exports) and a *detail* size with a
larger label budget (chart page, lightbox, `chart render --detail`).
